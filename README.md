...
2) Переключитесь на ветку `sprint-3`:
...
* Поля:
  ...
  При создании брони, бронь добавляется в очередь со статусом Pending. Каждые две секунды BookingBackgroundService проверяет есть ли сущности для обработки. Если есть, то берет их в обработку. На данный момм ент нет логики обработки заказов, так что спустя задержку в 5 секунд у брони выставляется статус Confirmed и записывается время когда обработка была завершена.
